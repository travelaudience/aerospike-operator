...
func testClusterSizeAfterScalingDownClusterWhileStartingAnother(tf *framework.TestFramework, ns *v1.Namespace, nodeCount int32) {
	...
	asc1, err := tf.AerospikeClient.AerospikeV1alpha2().AerospikeClusters(ns.Name).Create(context.TODO(), &aerospikeCluster1, metav1.CreateOptions{})
	...
	asc2, err := tf.AerospikeClient.AerospikeV1alpha2().AerospikeClusters(ns.Name).Create(context.TODO(), &aerospikeCluster2, metav1.CreateOptions{})
	Expect(err).NotTo(HaveOccurred())

	err = tf.WaitForClusterNodeCount(asc2, nodeCount)
	Expect(err).NotTo(HaveOccurred())

	asc1, err = tf.AerospikeClient.AerospikeV1alpha2().AerospikeClusters(asc1.Namespace).Get(context.TODO(), asc1.Name, metav1.GetOptions{})
	Expect(err).NotTo(HaveOccurred())
	Expect(asc1.Status.NodeCount).To(Equal(nodeCount))

	asc2, err = tf.AerospikeClient.AerospikeV1alpha2().AerospikeClusters(asc2.Namespace).Get(context.TODO(), asc2.Name, metav1.GetOptions{})
	Expect(err).NotTo(HaveOccurred())
