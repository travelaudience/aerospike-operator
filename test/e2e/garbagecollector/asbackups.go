...
func testDeleteExpiredAerospikeNamespaceBackup(tf *framework.TestFramework, ns *v1.Namespace, ttl string) {
	...
	go func(timeout time.Duration) {
		...
		for {
			select {
			case <-ticker.C:
				if _, err := tf.AerospikeClient.AerospikeV1alpha1().AerospikeNamespaceBackups(backup.Namespace).Get(backup.Name, metav1.GetOptions{}); errors.IsNotFound(err) {
					errCh <- nil
					return
				}
			case <-timer.C:
...
func testDoNotDeleteAerospikeNamespaceBackupWithoutTTL(tf *framework.TestFramework, ns *v1.Namespace) {
	...
	go func(timeout time.Duration) {
		...
		for {
			select {
			case <-ticker.C:
				if _, err := tf.AerospikeClient.AerospikeV1alpha1().AerospikeNamespaceBackups(backup.Namespace).Get(backup.Name, metav1.GetOptions{}); err != nil {
					errCh <- err
					return
				}
			case <-timer.C:
