...
created by the user according to a pre-established structure. Jobs discover pods
in the Aerospike cluster using the Aerospike service, then connecting to these
pods in order to backup and restore data for a given namespace.
...
This includes creating or deleting pods, creating the service used for
...
necessary service and managing the persistent volumes where namespace data will
be stored.
