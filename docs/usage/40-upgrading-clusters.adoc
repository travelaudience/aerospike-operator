...
*mandatory* backup of each of the Aerospike namespaces present in the target
Aerospike cluster. This is done in order to guarantee the safety of the data in
case of a major failure during the upgrade process. Hence, and before being able
to upgrade an Aerospike cluster, one must configure automatic pre-upgrade
backups for the target Aerospike cluster. This is done by making sure that the <<./20-backing-up-namespaces.adoc#aerospike-namespace-backup-prerequisites,pre-requisites>>
...
WARNING: Although `aerospike-operator` performs pre-upgrade backups of all the
Aerospike namespaces present in the target Aerospike cluster before actually
...
<<./30-restoring-namespaces.adoc#,restore>> each of the namespaces from the
pre-upgrade backup created as part of the upgrade process.
...
maximum data availability can only be ensured for namespaces having a
replication factor greater than one (i.e. having a value of
`.spec.replicationFactor` greater than one).
...
After a few moments, two `AerospikeNamespaceBackup` resources will have been
created (one per Aerospike namespace), and a `ClusterAutoBackupStarted`
condition will have been appended to the `AerospikeCluster` resource:
...
aerospike-namespace-1-4203-4204-upgrade   2m
----
[source,bash]
----
...
Depending on the size of each Aerospike namespace, it can take from a few
minutes to a few hours for these backups to complete. By the time the associated
jobs are complete, a `ClusterAutoBackupFinished` condition will be appended to
...
pre-upgrade backups made by `aerospike-operator` by following the steps detailed
